...
class PoseClassifier(object):
    ...
    def __call__(self, pose_landmarks):
        ...
        max_dist_heap.sort(key=lambda x: x[0])
        max_dist_heap = max_dist_heap[:self._top_n_by_max_distance]

        mean_dist_heap = [
            (min(
                np.mean(np.abs(self._pose_samples[idx].embedding - pose_embedding) * self._axes_weights),
                np.mean(np.abs(self._pose_samples[idx].embedding - flipped_pose_embedding) * self._axes_weights)
            ), idx)
            for _, idx in max_dist_heap
        ]

        mean_dist_heap.sort(key=lambda x: x[0])
